...
1. **`git init`** in `~/Desktop/azm/crm`, default branch `main`, remote `origin` set to
   ...
   **Commit identity — set repo-locally before the first commit.** The global identity is
   `Mustafa.abdullah <Mustafa.abdullah@azm.sa>`, a company address not registered on GitHub. Two
   problems: commits authored with an unregistered address do not link to a GitHub profile (no
   avatar, no contribution-graph credit — and criterion 3 is productivity, evidenced by this
   history), and on a **public** repo that work address sits in every commit for scrapers to harvest.
